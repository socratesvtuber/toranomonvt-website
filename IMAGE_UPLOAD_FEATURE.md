...
### Google フォームのファイルアップロードの制限について

Google フォームの仕様上、ファイルアップロードフィールドがある場合、フォーム編集画面で「1 つのファイルにのみ回答を許可する」設定になっています。この場合、同じファイルアップロードフィールドに複数のファイルをアップロードすることはできません。

**運用方法（変更済み）**:
- 初期設定では「1 回のみ回答可能」になっていましたが、これでは画像の差し替えができませんでした
- **複数回答可**の設定に変更しました
- 1 回目の回答：ヘッダー画像と全身画像をアップロード
- 2 回目以降の回答：画像を差し替えたい場合に再度アップロード

**複数回答可の設定方法**:
1. Google フォームの編集画面を開く
2. 設定（歯車アイコン）をクリック
3. 「回答数を 1 回に制限する」のチェックを**外す**
4. これでユーザーは何度でもフォームに回答可能になります

**代替案**:
- Firebase コンソールから直接画像 URL を手動で更新する
- Google Drive for Desktop を使用してファイル ID を維持したまま画像を差し替える
...
|------|----------|
| 2024-05-23 | 初版作成 |
| 2024-05-23 | Google フォームのファイルアップロードの制限についての注意点を追加 |
